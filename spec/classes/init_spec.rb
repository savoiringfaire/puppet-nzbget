...
describe 'nzbget' do
    ...
    on_supported_os.each do |os, facts|
      context "on #{os}" do
        ...
        context 'nzbget class without any parameters' do
          ...
          it { is_expected.to contain_file('/etc').that_comes_before('Concat[/etc/nzbget.conf]') }
          ...
          it { is_expected.to contain_file('/etc/init/nzbget.conf').that_notifies('Service[nzbget]') }
  ...
  context 'unsupported operating system' do
    describe 'nzbget class without any parameters on Solaris/Nexenta' do
      ...
      it { expect { is_expected.to contain_package('nzbget') }.to raise_error(Puppet::Error, %r{/Nexenta not supported/}) }
    end
  end
end
